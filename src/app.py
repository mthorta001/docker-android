...
def run():
    ...
    if is_first_run:
        ...
        cmd = 'emulator/emulator @{name} -port {ADB_PORT}-1 -gpu swiftshader_indirect -accel on -wipe-data -writable-system -verbose {custom_args}'.format(name=avd_name, custom_args=custom_args)
    else:
        logger.info('Using previously initialized AVD...')
        cmd = 'emulator/emulator @{name} -port {ADB_PORT}-1 -gpu swiftshader_indirect -accel on -verbose -writable-system {custom_args}'.format(name=avd_name, custom_args=custom_args)
