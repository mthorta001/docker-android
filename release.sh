...
IMAGE="${DOCKER_ORG:-rcswain}/docker-android"
...
function build() {
    ...
    for v in "${versions[@]}"; do
        ...
        docker build --no-cache -t $image_version --build-arg TOKEN=$TOKEN --build-arg ANDROID_VERSION=$v --build-arg API_LEVEL=$level \
